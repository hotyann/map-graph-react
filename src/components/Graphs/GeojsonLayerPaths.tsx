...
const GeojsonLayerPathsSample: React.FC = ({
  mapStyle = MAP_STYLE,
}: {
  mapStyle?: string;
}) => {
  const [accidents, setAccidents] = useState<Accident[] | undefined>(undefined);
  const [info, setInfo] = useState<PickingInfo<Road>>();
  const [year, setYear] = useState<number | undefined>(undefined);

  useEffect(() => {
    const fetchData = async () => {
      const data = (await load(DATA_URL.ACCIDENTS, CSVLoader))
        .data as Accident[];
      setAccidents(data);
      setYear(data[0]?.year);
    };
  ...
  const { incidents, fatalities } = useMemo(
    () => aggregateAccidents(accidents),
    [accidents]
  );
    ...
    new GeoJsonLayer<RoadProperties>({
      ...
      getLineColor: (f) => {
        ...
        const fatalitiesPer1KMile =
          ((fatalities[year!][key] || 0) / f.properties.length) * 1000;
      ...
      getLineWidth: (f) => {
        ...
        const incidentsPer1KMile =
          ((incidents[year!][key] || 0) / f.properties.length) * 1000;
  ...
  return (
    ...
    >
      ...
      {renderTooltip({ incidents, fatalities, year: year!, info })}
    </DeckGL>
  );
};

export default GeojsonLayerPathsSample;
